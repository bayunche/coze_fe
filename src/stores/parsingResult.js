...
export const useParsingResultStore = defineStore('parsingResult', () => {
   ...
   */
  const handleViewResultDetail = async (isSupplierMaterial = false) => {
    const workflowStore = useWorkflowStore();
    let taskIdToFetch = isSupplierMaterial ? workflowStore.supplierTaskId : workflowStore.taskId;
    let detailId = null;
    ...
    try {
      const workflowId = '7514898709852733475'; // 合同解析的实际工作流ID
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      const editPromises = payloads.map((item) => cozeParsingService.runEditWorkflow(editWorkflowId, item));
