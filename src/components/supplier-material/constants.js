...
📊 建议您检查AI匹配的数据是否是您期望的物资数据，如果不是请手动选择或直接进行新增并报中心审核。`,
  NO_MATCH: `🔍 AI智能搜索结果：
大模型已扫描全部数据库记录，暂未发现与您解析物资相匹配的数据项。
💡 建议从右侧按钮手动选择最接近的物资类型，如无合适物资价格信息，请另行新增物资信息并报中心审核。`,
  PRICE_NOT_FOUND: `💰 AI价格分析结果：
经过AI大模型智能分析，该季度该物资的符合该单位的价格信息在数据库中不存在。
🔧 建议您确认结算季度是否正确，或手动新增该季度的价格信息并报请中心进行审核。`,
  PRICE_MISMATCH: `💰 AI价格分析结果：
经过AI大模型智能分析，该季度该物资的符合该单位的价格信息与数据库中的数据不匹配。
🔧 建议您确认结算季度是否正确，或调整结算书相关数据。`
}
