...
export const REASON_EXPLANATIONS = {
  SIMILAR_MATCH: `🧠 AI大模型智能分析推荐：通过深度学习算法分析您的物资数据，在海量数据库中找到最匹配的相似物资。 📊 建议您仔细核对推荐的物资信息和价格数据是否符合预期，如需调整可重新选择。`,
  NO_MATCH: `🔍 AI智能搜索结果：大模型已扫描全部数据库记录，暂未发现与您解析物资相匹配的数据项。💡 建议从右侧按钮手动选择最接近的物资类型，或联系管理员录入新的标准数据。`
}
