// OwnerMaterialParsingResultDialog 组件工具函数
 ...
 * 查看任务详情
 * @param {Object} task - 任务对象
 * @param {Function} setSelectedTask - 设置选中任务的函数
 * @param {Function} setDialogVisible - 设置对话框可见性的函数
 */
export const viewTaskDetails = (task, setSelectedTask, setDialogVisible) => {
...
export const getTaskStatistics = (tasks) => {
  const safeData = getSafeTasksData(tasks)
  
  return {
    total: safeData.all.length,
    completed: safeData.completed.length,
    inProgress: safeData.inProgress.length,
    completionRate: safeData.all.length > 0 
      ? Math.round((safeData.completed.length / safeData.all.length) * 100) 
      : 0
  }
}