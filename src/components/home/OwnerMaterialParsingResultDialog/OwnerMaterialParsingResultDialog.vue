      ...
      >
        ...
        <template v-else-if="column.label === '进度'" #default="{ row }">
          <div style="display: flex; align-items: center">
            <el-progress
              ...
              :stroke-width="PROGRESS_CONFIG.STROKE_WIDTH"
              style="flex-grow: 1; margin-right: 10px"
            />
            <span :style="{ fontSize: '12px', color: PROGRESS_CONFIG.TEXT_COLOR }">
        ...
        <template v-else-if="column.label === '操作'" #default="{ row }">
          <el-button 
            type="primary" 
            size="small" 
            @click="() => viewTaskDetails(row, setSelectedTask, setDialogVisible)"
          >
...
const taskStatistics = computed(() => {
  return getTaskStatistics(safeTasksData.value)
})
