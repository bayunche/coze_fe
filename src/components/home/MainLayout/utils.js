 ...
 * @param {Function} selectFunction - 处理函数选择的方法
 * @param {Function} addMessage - 添加消息的方法
 */
export const processCustomFunctionSelect = (key, router, selectFunction, addMessage) => {
  if (key === SPECIAL_FUNCTIONS.SMART_BRAIN) {
    // 跳转到智能大脑页面
    router.push(ROUTES.SMART_BRAIN)
  } else {
    // 其他功能保持原有逻辑
    selectFunction(key, addMessage)
  }
}
 ...
 */
export const createCustomFunctionSelectHandler = (router, selectFunction, addMessage) => {
  return (key) => processCustomFunctionSelect(key, router, selectFunction, addMessage)
}